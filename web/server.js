...
module.exports = function(options) {
  ...
  var oauthDb = new OAuthDB(options.oauth_clients, options.authCodes, options
    .accessTokens);
  ...
  server.route([{
    ...
    config: {
      validate: {
        query: {
          ...
          action: Joi.string().optional().valid('signup', 'signin').default(
            'signin')
        }
      },
      pre: [{
        ...
        method: function(request, reply) {
          if (
            request.pre.client.allowed_responses.indexOf(request.query
              .response_type) === -1
          ) {
            return reply(Boom.forbidden(
              'Response type forbidden: ' + request.query.response_type
            ));
          }
          reply();
        }
        ...
        method: function(request, reply) {
          ...
          oauthDb.generateAuthCode(
            ...
            request.pre.user.id,
            request.pre.scopes,
            new Date(Date.now() + 60 * 100000000).toISOString(),
    ...
    handler: function(request, reply) {
      var redirectObj = url.parse(request.pre.client.redirect_uri,
        true);
    ...
    config: {
      validate: {
        payload: {
          grant_type: Joi.any().valid('authorization_code', 'password')
            .required(),
        ...
        failAction: function(request, reply, source, error) {
          reply(Boom.badRequest('invalid ' + source + ': ' + error.data
            .details[0].path));
        }
      },
      auth: false,
      ...
      pre: [{
        ...
        assign: 'client',
        method: function(request, reply) {
          oauthDb.getClient(request.payload.client_id, function(
            err, client) {
            if (err) {
              return reply(err);
            }
            if (
              client.allowed_grants.indexOf(request.pre.grant_type) ===
              -1 ||
              (
                request.pre.grant_type ===
                'authorization_code' &&
        ...
        method: function(request, reply) {
          if (request.pre.grant_type === 'password') {
            return server.methods.account.verifyPassword(request,
              function(err, json) {
                if (err) {
                  return reply(err);
                }
          ...
          oauthDb.verifyAuthCode(request.payload.code, request.pre
    ...
    config: {
      pre: [{
        assign: 'user',
        method: function(request, reply) {
          server.methods.account.verifyPassword(request, function(
            err, json) {
            if (err) {
              return reply(err);
            }
    ...
    handler: function(request, reply) {
      server.methods.account.resetPassword(request, function(err,
        json) {
        if (err) {
          return reply(err);
        }
    ...
    config: {
      ...
      pre: [{
        ...
        method: function(request, reply) {
          ...
          if (!result.strong) {
            var err = Boom.badRequest(
              'Password not strong enough.', result);
        ...
        assign: 'client',
        method: function(request, reply) {
          console.log(request.payload.client_id)
      ...
      server.methods.account.createUser(request, function(err, json) {
        ...
        if (json.login_error) {
          if (isUniqueError('username', json.login_error)) {
            ...
            return reply(Boom.badRequest(
              'An account exists for that email address'));
          }
    ...
    config: {
      ...
      pre: [{
        ...
        method: function(request, reply) {
          ...
          oauthDb.getClient(request.query.client_id, function(err,
            client) {
            if (err) {
              return reply(err);
            }
    ...
    config: {
      ...
      pre: [{
        ...
        assign: 'token',
        method: function(request, reply) {
          oauthDb.lookupAccessToken(request.pre.requestToken,
            function(err, token) {
              if (err) {
                return reply(err);
              }
              ...
              if (tokenScopes.indexOf('user') === -1 &&
                tokenScopes.indexOf('email') === -1) {
                return reply(Boom.unauthorized(
        ...
        assign: 'user',
        method: function(request, reply) {
          server.methods.account.getUser(request.pre.token.user_id,
            function(err, json) {
              if (err) {
    ...
    handler: function(request, reply) {
      server.methods.account.requestMigrateEmail(request, function(
        err, json) {
        if (err) {
          return reply(Boom.badImplementation(err));
        }
        reply({
          status: 'migration email sent'
        });
      });
    }
  }, {
    ...
    config: {
      ...
      pre: [{
        ...
        assign: 'isValidToken',
        method: function(request, reply) {
          server.methods.account.verifyToken(request, function(
            err, json) {
            if (err) {
              return reply(err);
            }
    ...
    handler: function(request, reply) {
      server.methods.account.checkUsername(request, function(err,
        json) {
        console.log(err)
        if (err) {
          return reply(err);
